...
String pid=request.getParameter("pid");
	...
	String uid=request.getParameter("assignee")==null ? session.getAttribute("id").toString() : request.getParameter("assignee");
    ...
    <div class="card shadow" style="min-height: 90vh">
        <div class="card-body">
        	...
        	<form class="float-right form-inline mr-2">
				...
				<select name="assignee" class="form-control form-control-sm mr-2">
					...
					%>
					<option <%= uid.equals(pp.get("employee_id"))?"selected":"" %> value="<%= pp.get("employee_id") %>">
					<%= pp.get("full_name").equals(user)?"Current User" : pp.get("full_name") %>
					</option>
					<% } %>
				</select>
...
<div class="modal fade" id="addpro">
    ...
    <div class="col-sm-8">
        <select name="uid" required class="form-control form-control-sm">
            ...
            <%
            List<Map<String,String>> employees = DbHelper.executeDQL(
            ...
            %>
                <option value="<%= emp.get("employee_id") %>" 
                    <%= emp.get("employee_id").equals(uid) ? "selected" : "" %>>
                    <%= emp.get("full_name") %>
                </option>
            <% } %>
        </select>
