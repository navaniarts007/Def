    ...
    <div class="card shadow" style="min-height: 90vh">
        <div class="card-body">
			...
			<table class="table table-sm">
				...
				<tbody>
					<%
					List<Map<String,String>> data=DbHelper.executeDQL("select t.id,t.title,p.title as ptitle,t.status,t.uid,t.priority,p.abbr,t.createdon from tasks t join task_group p on t.pid=p.id WHERE 1=?", "1");
		...
		<input type="hidden" name="role" value="<%= session.getAttribute("role") %>">
			...
			<div class="form-group">
					...
					<select name="pid" required class="form-control form-control-sm">
						...
						<%
						List<Map<String,String>> projs=DbHelper.executeDQL("SELECT id,title FROM task_group where 1=?", "1");
				...
				<div class="form-group">
					...
					<select name="uid" required class="form-control form-control-sm">
						...
						<%
						List<Map<String,String>> mgrs=DbHelper.executeDQL("SELECT id,uname,designation FROM users WHERE role!=? and id in(select uid from team WHERE pid in(SELECT pid from task_group where mgrid=?))", "A",session.getAttribute("id").toString());
